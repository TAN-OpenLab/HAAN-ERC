...
class DropScaledDotProductAttention(nn.Module):
    ...
    def forward(self, q, k, v, d0, d1, mask=None, pre_decisions=None):
        ...
        decisions = distributions[:, :, :, :, -1] * -1e7  # [b x 1 x lq x lq]
        attn = torch.matmul(q / self.temperature, k.transpose(2, 3))
        if mask is not None:
            attn = attn + decisions
            attn = attn.masked_fill(mask == 0, -1e9)
...
class SMModel(nn.Module):
    ...
    def __init__(
        ...
        self.scale_emb = scale_emb
        self.selector = selector
        self.smax_fc = nn.Linear(2 * d_model, n_classes)
        ...
        self.se = SpeakerEncoder(n_classes, 2 * d_model, n_layers, n_head, d_k, d_v, 2 * d_model, d_inner, dropout,
                                 self.selector, args)
        self.ce = SpeakerEncoder(n_classes, 2 * d_model, n_layers, n_head, d_k, d_v, 2 * d_model, d_inner, dropout,
                                 self.selector, args)
        self.ge = SpeakerEncoder(n_classes, 2 * d_model, n_layers, n_head, d_k, d_v, 2 * d_model, d_inner, dropout,
                                 False, args)
